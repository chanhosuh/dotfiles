...
tags = 
{
    names = { "main", "aux", "www", "mail", "im", "foo", "bar" },
    layout = { layouts[6], layouts[6], layouts[8], layouts[8], layouts[2], layouts[1], layouts[1] }
}
for s = 1, screen.count() do
    -- Each screen has its own tag table.
    tags[s] = awful.tag(tags.names, s, tags.layout)
    -- set IM client to smaller width on right of "mail" tag
    -- awful.tag.setproperty(tags[s][3], "mwfact", 0.72)
end
-- }}}
...
myawesomemenu = {
   ...
   { "edit config", "xfce4-terminal -x vim /home/suh/.config/awesome/rc.lua" },
...
settings = {
    ...
    --{ "Display settings", "nvidia-settings"}, -- Nvidia driver settings
...
globalkeys = awful.util.table.join(
    ...
    -- Map media keys to gMusicbrowser
...
awful.util.spawn_with_shell("syndaemon -k -i 2 -d&")
awful.util.spawn_with_shell("xfsettingsd")
awful.util.spawn_with_shell("xfce4-power-manager &")
-- awful.util.spawn("xcompmgr")
awful.util.spawn_with_shell("xcalib -gc 1.8 ~/mac_colorprofile.icc")
...
awful.util.spawn_with_shell("xmodmap -e 'keycode 104 = Alt_L'")
-- awful.util.spawn_with_shell("run-once.sh guake")
